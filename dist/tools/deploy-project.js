...
async function lookupProject(dbUrl, projectId) {
    return withDeployClient(dbUrl, async (client) => {
        const result = await client.query('SELECT build_cmd, deploy_cmd, smoke_url, working_dir, default_container FROM projects WHERE project_id = $1', [projectId]);
...
async function detectAndRegisterProject(dbUrl, projectId) {
    ...
    if (!workingDir) {
        throw new Error(`Cannot auto-detect: no directory found for project_id '${projectId}'. Tried: ${candidateDirs.join(', ')}`);
    }
    ...
    if (hasSwarmYml) {
        ...
        throw new Error(`Cannot auto-detect deploy type for '${projectId}' in ${workingDir}. No swarm.yml, Dockerfile, package.json, or requirements.txt found.`);
    }
    await withDeployClient(dbUrl, async (client) => {
        await client.query(`INSERT INTO projects (project_id, build_cmd, deploy_cmd, smoke_url, working_dir, created_at, updated_at)
       ...
       ON CONFLICT (project_id) DO UPDATE SET
         ...
         updated_at = now()`, [projectId, buildCmd, deployCmd, smokeUrl || null, workingDir]);
    });
    return {
        build_cmd: buildCmd,
...
async function runLocalCmd(cmd) {
...
async function smokeTest(url) {
    ...
    for (let i = 1; i <= 12; i++) {
        try {
            ...
            if (result.ok)
                return `passed (HTTP ${result.status}, attempt ${i})`;
        }
        catch { /* retry */ }
        if (i < 12)
