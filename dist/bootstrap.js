...
async function bootstrapSession(params) {
    ...
    try {
        const existing = await (0, db_js_1.withDbClient)(dbUrl, async (client) => {
            const r = await client.query(`SELECT session_id FROM sessions WHERE user_id = $1 AND project_id = $2 AND status = 'active' AND created_at > now() - interval '4 hours' ORDER BY created_at DESC LIMIT 1`, [user_id, projectId]);
            return r.rows[0] ?? null;
        });
        if (existing) {
            ...
            const agentRunning = await (0, db_js_1.withDbClient)(dbUrl, async (client) => {
                const r2 = await client.query(`SELECT 1 FROM session_messages WHERE session_id = $1 AND role = 'coding_agent' LIMIT 1`, [existing.session_id]);
    ...
    try {
        ...
        (0, code_task_js_1.spawnCodeTask)({ instruction, workingDir, sessionId, dbUrl, allowedTools, permissionMode: 'bypassPermissions' });
