...
async function buildBootstrapInstruction(sessionId, dbUrl) {
    ...
    if (sessionWorktreePath && sessionBranch) {
        if (!fs.existsSync(sessionWorktreePath)) {
            ...
            const r = (0, child_process_1.spawnSync)("git", ["-C", repoPath, "worktree", "add", "-b", sessionBranch, sessionWorktreePath, "HEAD"], { encoding: "utf8" });
            if (r.status !== 0) {
                // Branch already exists — attach without -b
                (0, child_process_1.spawnSync)("git", ["-C", repoPath, "worktree", "add", sessionWorktreePath, sessionBranch], { encoding: "utf8" });
            }
        }
...
async function bootstrapSession(params) {
    ...
    try {
        const existing = await (0, db_js_1.withDbClient)(dbUrl, async (client) => {
            const r = await client.query(`SELECT session_id FROM sessions WHERE user_id = $1 AND project_id = $2 AND status = 'active' AND created_at > now() - interval '4 hours' ORDER BY created_at DESC LIMIT 1`, [user_id_resolved, projectId]);
            return r.rows[0] ?? null;
        });
        if (existing) {
            if (model) {
            ...
            if (process.env.ANTHROPIC_API_KEY) {
                ...
                if (fs2.existsSync(claudeJsonPath)) {
                    try {
                        ...
                        if (cj.primaryApiKey)
                            authHint = `${triggeredByName} Anthropic API key (claude.json)`;
                    }
                    catch { }
                }
            }
            await client.query(`INSERT INTO sessions (session_id, project_id, container, repo, status, session_type, title, prompt_preview, jira_issue_keys, user_id, triggered_by_name, triggered_by_slack_user_id, slack_thread_url, branch, worktree_path, auth_hint, model, gateway_parent_key, created_at, updated_at)
         VALUES ($1, $2, $3, $4, 'pending', 'dev', $5, $6, $7::text[], $8, $9, $10, $11, $12, $13, $14, $15, $16, now(), now())`, [sessionId, projectId, projConfig.default_container ?? "dev-david", projectId,
    ...
    // Use /tools/invoke with sessions_spawn instead, same pattern as listen-chain.ts.
    {
        ...
        try {
            const spawnMessage = await (0, db_js_1.buildSpawnMessage)(sessionId, dbUrl, undefined);
