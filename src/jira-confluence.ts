...
export async function jiraAuthHeaders(): Promise<Record<string, string>> {
...
export async function fetchJiraIssue(issueKey: string): Promise<{ updated: string; description: string; summary: string }> {
...
export async function logCacheWarning(dbUrl: string, message: string): Promise<void> {
  try {
...
export async function fetchConfluencePage(pageId: string): Promise<{ versionNumber: number; versionWhen: string; bodyHtml: string }> {
  try {
...
export async function populateCacheForProject(
  dbUrl: string,
  jiraKeys: string[],
...
export async function searchJiraForIssue(projectKey: string, keywords: string): Promise<string | null> {
  const baseUrl = (process.env.JIRA_URL ?? "").replace(/\/$/, "");
...
export async function createJiraTaskIssue(
  projectKey: string,
  summary: string,
  description: string
): Promise<string | null> {
