...
async function postToFeed(sessionId: string, dbUrl: string, content: string, messageType = "execution_update", role = "dev_lead"): Promise<void> {
    ...
    try {
      switch (name) {
        ...
        case "start_session": {
          ...
          if (rules) fs.writeFileSync(rulesFile, rules);
          ...
          if (rules) claudeArgs.push("--append-system-prompt-file", rulesFile);
          ...
          proc.stdout?.on("data", (chunk: Buffer) => {
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (usage) {
                  if (usage.input_tokens) session.tokenInput += usage.input_tokens;
                  if (usage.output_tokens) session.tokenOutput += usage.output_tokens;
                  if (usage.cache_read_input_tokens) session.tokenCache += usage.cache_read_input_tokens;
                  if (usage.cache_creation_input_tokens) session.tokenCache += usage.cache_creation_input_tokens;
                }

                // Model name
                if (evt.message?.model) session.model = evt.message.model;
          ...
          proc.stderr?.on("data", (chunk: Buffer) => {
            ...
            if (text) console.error(`[session:${job_id}] stderr:`, text);
          });
          ...
          proc.on("close", (code) => {
            ...
            try { if (rules) fs.unlinkSync(rulesFile); } catch {}
          });
        ...
        case "pause_session": {
          ...
          const s = cliSessions.get(job_id);
          if (!s) throw new Error(`Session ${job_id} not found`);
          if (s.status !== "running") throw new Error(`Session ${job_id} is not running (status: ${s.status})`);
          s.proc.kill("SIGSTOP");
          s.status = "paused";
        ...
        case "resume_session": {
          ...
          const s = cliSessions.get(job_id);
          if (!s) throw new Error(`Session ${job_id} not found`);
          if (s.status !== "paused") throw new Error(`Session ${job_id} is not paused (status: ${s.status})`);
          s.proc.kill("SIGCONT");
          s.status = "running";
        ...
        case "send_message": {
          ...
          const { job_id, message } = args as any;
          const s = cliSessions.get(job_id);
          if (!s) throw new Error(`Session ${job_id} not found`);
          if (s.status !== "running") throw new Error(`Session ${job_id} is not running (status: ${s.status})`);
          if (!s.proc.stdin) throw new Error(`Session ${job_id} stdin not available`);
