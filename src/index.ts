  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
      switch (name) {
        ...
        case "spawn_dev_lead": {
          // POST to gateway /tools/invoke (async) to spawn dev-lead
          ...
          try {
            ...
            return { content: [{ type: "text", text: JSON.stringify({ ok: true, session_id: sessionId, childSessionKey: parsed.childSessionKey }) }] };
          } catch (fetchErr: any) {
            return { content: [{ type: "text", text: JSON.stringify({ ok: false, error: (fetchErr as Error).message }) }] };
          }
        }
        ...
        case "create_session": {
          ...
          try {
            ...
            // Step 3: Spawn dev-lead via gateway /tools/invoke (async — returns immediately with childSessionKey)
            const gatewayUrl = process.env.OPENCLAW_GATEWAY_URL ?? "http://172.17.0.1:18789";
            const gatewayToken = process.env.OPENCLAW_GATEWAY_TOKEN ?? "";
            let spawnOk = false;
            let spawnError = "";
            ...
            try {
              ...
              if (!resp.ok) {
                ...
                const spawnParsed = await resp.json() as any;
                childSessionKey = spawnParsed.childSessionKey ?? null;
                spawnOk = true;
              }
            } catch (fetchErr: any) {
              spawnError = (fetchErr as Error).message;
            }
    ...
    listenClient.on("notification", (msg) => {
      void (async () => {
        try {
          ...
          const isChat = messageType === "chat" && payload.role !== "dev_lead" && payload.role !== "coding_agent";
          if (!isTaskBrief && !isApprovalResponse && !isChat) return;

          // Skip interactive sessions — they use chat_session directly, not dev-lead
          // Only respawn on approval_response or chat for active sessions.
          try {
            ...
            if (checkRes.rows.length > 0) {
              ...
              if ((isApprovalResponse || isChat) && session.status !== "active") {
                console.log(`[listen-chain] skip wake for non-active session ${sessionId} (${session.status})`);
                return;
              }
            }
          } catch (e: any) {
